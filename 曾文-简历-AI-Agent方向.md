...
| **后端框架** | FastAPI 0.115、Spring Boot 3.4、Spring AI 1.0、MyBatis-Plus |
| **数据库** | PostgreSQL 16（asyncpg）、MySQL 8.0、SQLite、Elasticsearch、Redis 7 |
| **中间件** | Kafka、Redis |
| **AI/LLM** | LangGraph4j、OpenAI / DeepSeek / Qwen / ZhiPu 等 OpenAI-compatible 多提供商、Context Engineering、Prompt 分层架构 |
| **工具** | Docker、Git、Alembic、Linux、JWT、微信支付/商户、MinIO |

---

## 工作经历

### 荆州大宅门装饰 | 后端开发工程师（Python/FastAPI） 2025.04 - 至今

负责家装行业分销与线索运营平台「大宅门推客系统」的后端架构设计与开发，覆盖多角色权限体系、线索全生命周期状态机、奖励佣金链路与微信支付；同时设计并落地企微 AI 机器人的数据图表直查系统。

### 博士眼镜(海南)投资控股有限公司 | 运营数据分析与系统流程专员 2023.09 - 2025.11
...
运行在终端中的 AI Agent CLI，面向真实项目开发场景。具备 ReAct、Plan-and-Execute、Multi-Agent 协作、MCP、轻量代码索引、记忆系统、快照恢复、Runtime API 等能力，覆盖 Agent 工程化的核心能力。
...
- **Plan-and-Execute 模式**：独立 Planner 生成 DAG 任务图，按依赖关系分批并行执行；工具执行层支持 HITL 审批与写操作审计，确保关键操作安全可控
- **Multi-Agent 协作**：Planner（任务分解）、Worker（执行）、Reviewer（审查）三种 Agent 角色，支持依赖调度、并行 Worker 和 Review 重试机制
- **MCP 客户端与服务端**：支持 stdio 和 Streamable HTTP 两种传输方式的 MCP Server 接入；Agent 自身也可作为 MCP Server 暴露内置工具
- **轻量代码索引**：基于 SQLite 的本地代码行级索引，按文件/行分块并做关键词检索，快速定位代码片段，作为 RAG 思想的工程化落地
- **记忆系统**：短期记忆管理对话上下文；长期记忆通过 SQLite 持久化关键事实，跨会话复用；Agent Run 前后自动创建快照，支持恢复现场
- **Skill 系统**：支持内置/用户级/项目级 Skill，三级渐进式加载策略
- **Runtime API**：提供线程、turn（对话轮次）、事件日志和持久化后台任务能力
...
- **多 LLM 提供商集成**：通过 Spring AI ChatClientFactory 动态创建客户端，统一接入 OpenAI、DeepSeek、Qwen、ZhiPu 等 OpenAI-compatible 大模型
...
- **RAG 全流程实现**：文档解析（Apache Tika 多格式支持）→ Chunk → Embedding（通过可配置 OpenAI-compatible API 远程生成）→ Elasticsearch 索引 → 语义检索 → LLM 生成带引用回答
- **实时数据同步**：Kafka 消息队列异步处理文档索引，WebSocket 实时推送索引进度与问答结果
- **多租户架构**：Spring Security + JWT 鉴权体系，MinIO 对象存储，Redis 缓存加速
- **多模型兼容**：LLM 支持 DeepSeek / Qwen / ZhiPu 等 OpenAI-compatible 提供商；Embedding 支持阿里云、智谱等远程 API
- **技术栈**：Spring Boot 3.4 / Java 17 / Elasticsearch / Kafka / Spring Data JPA / Vue 3 / TypeScript / MinIO / Docker

### 大宅门推客系统 v2 — 家装分销与线索运营平台（公司真实项目） 2025.04 - 至今
...
- **线索状态机**：将线索状态从单值重构为 17 位状态标记（核心状态位 + 奖励关联位），奖励触发天然幂等，主管恢复不重复发奖
...
- **技术栈**：Python 3.12 / FastAPI 0.115 / Pydantic v2 / SQLAlchemy 2.0 async / asyncpg / PostgreSQL 16 / Redis 7 / Alembic / 微信支付
...
- **Python/Java 双栈交付**：Python 端（FastAPI 异步栈 + Agent CLI 完整工程）与 Java 端（Spring Boot 后端工程 + LangGraph4j 编排引擎 + RAG 知识库）均具备独立交付公司级项目的能力
...
- **工程化与资金安全**：在大宅门推客系统中落地支付对账、状态机幂等、数据隔离与 Alembic 生产迁移等实践，理解真实业务中的资金一致性与工程可靠性问题
